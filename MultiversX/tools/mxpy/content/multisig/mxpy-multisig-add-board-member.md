# mxpy multisig add-board-member
...
```bash
mxpy multisig add-board-member --board-member BOARD_MEMBER --contract CONTRACT [options]
```

## Parameters

- `--board-member BOARD_MEMBER`  
  Bech32 address of the proposed board member.
...
```bash
mxpy multisig add-board-member --board-member bob --contract msig1 --sender alice --send
