# mxpy staking-provider unstake-nodes

Unstake nodes from a delegation contract. Must be called by the contract owner.

## Usage

```bash
mxpy staking-provider unstake-nodes [options]
```
...
```bash
mxpy staking-provider unstake-nodes --bls-keys bls1,bls2 --delegation-contract erd1... --proxy https://devnet-gateway.multiversx.com --pem owner.pem --send
