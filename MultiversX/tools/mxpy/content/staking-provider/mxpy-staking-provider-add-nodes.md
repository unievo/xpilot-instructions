...
Add new nodes to a delegation contract. Must be called by the contract owner.

## Usage

```bash
mxpy staking-provider add-nodes [options]
```
...
- `--delegation-contract DELEGATION_CONTRACT`  
  String. Bech32 address of the delegation contract.
- `--proxy PROXY`  
...
```bash
mxpy staking-provider add-nodes --validators-pem nodes.pem --delegation-contract erd1... --proxy https://devnet-gateway.multiversx.com --pem owner.pem --send
