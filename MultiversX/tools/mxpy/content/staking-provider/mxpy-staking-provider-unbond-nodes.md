# mxpy staking-provider unbond-nodes

Unbond nodes from a delegation contract. Must be called by the contract owner.

## Usage

```bash
mxpy staking-provider unbond-nodes [options]
```
...
```bash
mxpy staking-provider unbond-nodes --bls-keys bls1,bls2 --delegation-contract erd1... --proxy https://devnet-gateway.multiversx.com --pem owner.pem --send
