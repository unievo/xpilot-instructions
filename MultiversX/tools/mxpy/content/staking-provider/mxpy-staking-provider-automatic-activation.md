# mxpy staking-provider automatic-activation

Set or unset automatic activation for a delegation contract. Must be called by the contract owner.

## Usage

```bash
mxpy staking-provider automatic-activation [options]
```

## Parameters

- `--set`  
  Flag. Set automatic activation to True.
- `--unset`  
  Flag. Set automatic activation to False.
...
```bash
mxpy staking-provider automatic-activation --set --delegation-contract erd1... --proxy https://devnet-gateway.multiversx.com --pem owner.pem --send
