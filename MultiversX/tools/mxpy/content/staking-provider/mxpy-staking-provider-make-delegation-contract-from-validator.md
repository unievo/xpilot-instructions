# mxpy staking-provider make-delegation-contract-from-validator
...
- `--max-cap MAX_CAP`  
  Integer. Total delegation cap in EGLD, fully denominated. Use value 0 for uncapped.
- `--fee FEE`  
  Integer. Service fee as hundredths of percents (e.g. 3745 means 37.45%).
- `--proxy PROXY`  
...
```bash
mxpy staking-provider make-delegation-contract-from-validator --max-cap 1000 --fee 3745 --proxy https://devnet-gateway.multiversx.com --pem validator.pem --send
