...
```bash
mxpy validator unstake-tokens [options]
```
...
- `--relayer-wallet-index RELAYER_WALLET_INDEX`  
  int, the address index; can be used for PEM files, keyfiles of type mnemonic or Ledger devices (default: 0)
- `--unstake-value UNSTAKE_VALUE`  
  int, the unstake value

## Example

```bash
mxpy validator unstake-tokens --proxy https://gateway.example.com --pem wallet.pem --unstake-value 1000000000000000000 --send
